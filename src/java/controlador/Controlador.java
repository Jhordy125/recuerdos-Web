                ...
                try {
                    ...
                    for (int i = 0; i < items.size(); i++) {
                        ...
                        if (!fileItem.isFormField()) {
                            File file = new File("C:\\Users\\Public\\carrito\\Pagina Web Centica\\Proyecto Centica\\carrito\\" + fileItem.getName());
                            fileItem.write(file);
                            p.setImagen("http://localhost:48422/carrito/" + fileItem.getName());
                        } else {
                ...
                if (p.getId() != 0) {
                   try {
                       pdao.Eliminar(idProducto);
                        request.getRequestDispatcher("Controlador?accion=Eliminar").forward(request, response);
                    } catch (Exception e) {
                        request.setAttribute("no se elimino" + e.getMessage(), e);
                    }
                } else {
                    request.setAttribute("msg","no hay producto" );
        ...
        if (request.getParameter("id")!=null) {
           p.setId(Integer.parseInt(request.getParameter("id")));
            try {
                ...
                request.setAttribute("msg","no se agrero"+ e.getMessage());
                                
            }
        } else {
        request.setAttribute("msg","no se encontro produto");
        }
